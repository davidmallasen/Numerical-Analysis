...
% Metodo de prediccion-correccion. Metodo predictor: Adams-Bashforth de 4
% pasos. Metodo corrector: Adams-Moulton de 4 pasos. Inicializacion: Metodo
...
for i = 1:N - r + 1
    ...
    % Prediccion
    x(:, i + 4) = x(:, i + 3) + (h / 24) * (55 * ev(:, j(3)) - 59 * ev(:, j(2)) + 37 * ev(:, j(1)) - 9 * ev(:, j0));
    
    % Evaluacion
    tmp_ev = f(t(i + r), x(:, i + r));
    
    % Correccion
    x(:, i + 4) = x(:, i + 3) + (h / 720) * (251 * tmp_ev + 646 * ev(:, j(3)) - 264 * ev(:, j(2)) + 106 * ev(:, j(1)) - 19 * ev(:, j0));
end
