...
% Metodo de prediccion-correccion. Metodo predictor: Adams-Bashforth de 2
% pasos. Metodo corrector: Adams-Moulton de 2 pasos. Inicializacion: Metodo
% de Euler mejorado.
%
% La funcion mab2am2 resuelve un problema de valor inicial de la forma
% x' = f(t, x) en [t0, T]
% x(t0) = x0,
...
for i = 1:N - r + 1
    ...
    % Correccion
    x(:, i + 2) = x(:, i + 1) + (h / 12) * (5 * tmp_ev + 8 * ev(:, j(1)) - ev(:, j0));
end
