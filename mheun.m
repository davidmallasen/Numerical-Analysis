function [t, x] = mheun(f, intervalo, x0, N)
% Heun method.
%
% La funcion mheun resuelve un problema de valor inicial de la forma
% x'=f(t,x) en [t0,T]
% x(t0)=x0,
% con x0 en R^n, mediante el metodo de Heun.
%
% ENTRADA:
...
for i = 1:N
    F1 = f(t(i), x(:, i));
    F2 = f(t(i) + (2*h / 3), x(:, i) + (2*h / 3) * F1);
    x(:, i + 1) = x(:, i) + (h/4) * (F1 + 3*F2);
end
